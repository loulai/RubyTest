...
def select_elements_starting_with_vowel(array)
	...
	downcased_array.select {|elem| vowels.include? (elem.chars.first) }
end
...
def remove_nils_and_false_from_array(array)
	array.reject {|elem| elem == nil || elem == false}
end
...
def get_first_half_of_string(string)
	number_of_characters_in_string = string.chars.count
	half_num = (number_of_characters_in_string/2.0).ceil
	string.slice(0, half_num)
end
...
def number_of_elements_that_are_palindromes(array)
	array_of_palindromes = array.select {|word| word == word.reverse}
	array_of_palindromes.count
end
...
def average_of_array(array)
	total = array.inject(:+)
	number_of_numbers = array.count.to_f
	(total/number_of_numbers).round
end
...
def convert_array_to_a_hash(array)
	separated_array = []
	array.each_slice(2) {|x| separated_array << x}
	separated_array.to_h
end
...
def your_birthday_is_on_a_friday_in_the_year(birthday)
	...
	full_date = years.last + (60*60*24*365)
	full_date.year
end
